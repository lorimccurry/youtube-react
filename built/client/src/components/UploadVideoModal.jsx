...
function UploadVideoModal(_a) {
    ...
    return (<UploadVideoModal_1.default>
      <div className="modal-content">
        <div className="modal-header">
          ...
          <div style={{ display: url ? 'block' : 'none' }}>
            <Button_1.default onClick={handleTab}>
              {tab === 'PREVIEW' ? 'Next' : 'Upload'}
            </Button_1.default>
          </div>
        </div>

        {tab === 'PREVIEW' && (<div className="tab video-preview">
            <VideoPlayer_1.default previewUrl={previewVideo} video={url}/>
          </div>)}

        {tab === 'FORM' && (<div className="tab video-form">
            <h2>Video Details</h2>
            <input type="text" placeholder="Enter your video title" value={title} onChange={function (event) { return setTitle(event.target.value); }}/>
            <textarea placeholder="Tell viewers about your video" value={description} onChange={function (event) { return setDescription(event.target.value); }}/>
          </div>)}
      </div>
    </UploadVideoModal_1.default>);
}
